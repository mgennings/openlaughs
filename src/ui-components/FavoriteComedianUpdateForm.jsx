...
import { getFavoriteComedian } from "../graphql/queries";
import { updateFavoriteComedian } from "../graphql/mutations";
const client = generateClient();
export default function FavoriteComedianUpdateForm(props) {
  const {
    id: idProp,
    favoriteComedian: favoriteComedianModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = favoriteComedianRecord
      ? { ...initialValues, ...favoriteComedianRecord }
      : initialValues;
  ...
  const [favoriteComedianRecord, setFavoriteComedianRecord] = React.useState(
    favoriteComedianModelProp
  );
  React.useEffect(() => {
      ...
      const record = idProp
        ? (
            await client.graphql({
              query: getFavoriteComedian.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getFavoriteComedian
        : favoriteComedianModelProp;
      setFavoriteComedianRecord(record);
    };
    queryData();
  }, [idProp, favoriteComedianModelProp]);
  React.useEffect(resetStateValues, [favoriteComedianRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateFavoriteComedian.replaceAll("__typename", ""),
            variables: {
              input: {
                id: favoriteComedianRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "FavoriteComedianUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || favoriteComedianModelProp)}

...
import { getShowRSVP } from "../graphql/queries";
import { updateShowRSVP } from "../graphql/mutations";
const client = generateClient();
export default function ShowRSVPUpdateForm(props) {
  const {
    id: idProp,
    showRSVP: showRSVPModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = showRSVPRecord
      ? { ...initialValues, ...showRSVPRecord }
      : initialValues;
  ...
  const [showRSVPRecord, setShowRSVPRecord] = React.useState(showRSVPModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getShowRSVP.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getShowRSVP
        : showRSVPModelProp;
      setShowRSVPRecord(record);
    };
    queryData();
  }, [idProp, showRSVPModelProp]);
  React.useEffect(resetStateValues, [showRSVPRecord]);
  const validations = {
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateShowRSVP.replaceAll("__typename", ""),
            variables: {
              input: {
                id: showRSVPRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ShowRSVPUpdateForm")}
      {...rest}
    >
      <TextField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || showRSVPModelProp)}
